...
(function(host) {
    ...
    function createCityTopHtml() {
        ...
        p.appendChild(a);
        for (var row in city) {
            if (city.hasOwnProperty(row)) { a = doc.createElement("a");
                a.href = "#" + row;
    ...
    function createHotCityHtml() {
        ...
        fragment.appendChild(strong);
        for (var i = 0, len = hotCity.length; i < len; i++) { currentCity = hotCity[i];
        ...
        for (var miao in city) {
            if (city.hasOwnProperty(miao)) {
                var tr = doc.createElement("tr"),
                    ...
                    currentMiao = null,
                    a = null;
                for (var j = 0, len = miaoArray.length; j < len; j++) { currentMiao = miaoArray[j];
                    a = doc.createElement("a");
    ...
    function addKeyPressEvent(tag) {
        tag.onkeypress = function(e) {
            var event = window.event || e,
                charCode, keyIndex = 0,
                keyCode;
            if (typeof event.charCode == "number") { charCode = event.charCode; } else { charCode = event.keyCode; }
    ...
    var citypicker = {
        ...
        show: function() {
            ...
            if (lenght == 2) { point = arguments[0];
                callback = arguments[1]; } else { options = arguments[0];
                if (options && typeof options === "object") { point.left = options.left || citypicker.point;
            ...
            if (!popUpCityFrame || !docCityFrame) { popUpCityFrame = createFrame();
        ...
        close: function() {
            if (popUpCityFrame) { popUpCityFrame.style.display = "none";
        ...
        fix: function(point) {
            if (point && typeof point === "object") {
                ...
                if (popUpCityFrame) { popUpCityFrame.style.left = left + "px";
                    popUpCityFrame.style.top = top + "px"; } }
            return this;
        },
        bind: function(callback) {
            if (toString.call(callback) === "[object Function]") { citypicker.callback = callback;
                if (popUpCityFrame !== null) { addSelectEvent(selectEventProxy, "click", callback);
